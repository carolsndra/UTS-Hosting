...
// Usage: Include this script after profile modal HTML
...
(function () {
  ...
  function initCrop() {
    ...
    function renderAvatarEverywhere() {
      ...
      const profilePlaceholderEl = document.getElementById("profileAvatarPlaceholder");

      if (profileAvatarEl && profilePlaceholderEl) {
        if (avatar) {
          profileAvatarEl.src = `${API}/uploads/${encodeURIComponent(avatar)}`;
      ...
      // --- Sidebar/Topbar avatar (profile button bulat) ---
      const profileBtn = document.getElementById("profileBtn");
      if (profileBtn) {
        if (avatar) {
          const url = `${API}/uploads/${encodeURIComponent(avatar)}`;
          profileBtn.innerHTML = `<img src="${url}" class="w-full h-full object-cover rounded-full" />`;
        } else {
          const initial =
            (u.username || u.email || "U").toString().trim()[0]?.toUpperCase() || "U";
          profileBtn.innerHTML = `<span class="font-semibold">${initial}</span>`;
        }
      }
    }

    // expose biar file lain bisa panggil setelah PATCH /login/profile berhasil
    window.renderAvatarEverywhere = renderAvatarEverywhere;
    ...
    function openCropModal(file) {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          if (!currentCropImage || !currentCropContainer || !currentModal) return;
          ...
          setTimeout(() => {
            const currentCropImage = document.getElementById("cropImage");
            const currentCropContainer = document.getElementById("cropContainer");
            if (!currentCropImage || !currentCropContainer) return;

            const containerSize =
              currentCropContainer.offsetWidth || currentCropContainer.clientWidth || 300;
        ...
        img.onerror = () => alert("Gagal memuat gambar. Pastikan file adalah gambar yang valid.");
        img.src = e.target.result;
      };

      reader.onerror = () => alert("Gagal membaca file. Pastikan file adalah gambar yang valid.");
    ...
    document.addEventListener("mousemove", (e) => {
      if (!isDragging || !cropImageElement) return;
    ...
    document.addEventListener("mouseup", () => {
      ...
      const currentCropContainer = document.getElementById("cropContainer");
      if (currentCropContainer) currentCropContainer.style.cursor = "move";
    });

    // prevent default drag on img
    if (cropImage) {
      cropImage.addEventListener("mousedown", (e) => e.preventDefault());
    }
    ...
    if (cropContainer) {
      cropContainer.addEventListener("wheel", (e) => {
        ...
        const minScale =
          Math.min(containerSize / cropImageElement.width, containerSize / cropImageElement.height) *
          0.8;
    ...
    async function cropImageToCircle() {
      ...
      const sourceX = Math.max(0, (0 - imgLeft) / cropScale);
      const sourceY = Math.max(0, (0 - imgTop) / cropScale);
      const sourceRight = Math.min(cropImageElement.width, (containerSize - imgLeft) / cropScale);
      const sourceBottom = Math.min(cropImageElement.height, (containerSize - imgTop) / cropScale);
    ...
    if (cropSave) {
      cropSave.addEventListener("click", async () => {
        ...
        if (blob) {
          ...
          // Update preview modal only (server upload terjadi pas tombol SAVE profile di modal profile)
          const reader = new FileReader();
          reader.onload = () => {
            // render ulang: minimal placeholder hilang, dsb
            renderAvatarEverywhere();
          };
